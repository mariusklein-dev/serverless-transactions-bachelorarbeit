...
json_file_path = '2pc_parallel_noinstance_success.json'
...
plt.title('Sequentiell paralleles 2PC Latenztestszenario ohne Umgebungsparameter mit Ausreißereliminierung, erfolgreich')
...
plt.figtext(0.10, 0.82, f"Durchschnitt der Kaltstart Latenz: {first_batch_average:.2f} ms\n"
                        f"Kürzester Kaltstart: {first_batch_min} ms\n"
                        f"Längster Kaltstart: {first_batch_max} ms\n"
                        f"Eliminierte Batches: 1 & 2",
            fontsize=10, bbox=dict(facecolor='lightgrey', alpha=0.5))
